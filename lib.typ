...
#let tree(tag, ..children, child-spacing: 1em, layer-spacing: 2.3em, roof: false, term: false, stroke: 0.75pt) = {
...
#let syntree(code, terminal: (:), nonterminal: (:), child-spacing: 1em, layer-spacing: 2.3em) = {
  ...
  let term_stack = (false,)
  for token in code.matches(regex(`(\\\[|\\\]|[^\[\]\s])+|\[|\]`.text)) {
    if token.text == "[" {
      stack.push(())
      roof_stack.push(false)
      term_stack.push(false)
    } else if token.text == "]" {
      let (tag, ..children) = stack.pop()
      let roof = roof_stack.pop()
      let term = term_stack.pop()
      if roof {
        children = (text(..terminal, children.join([ ])),)
      } else if term {
        tag = text(tag + linebreak() + children.join())
      }
      stack.last().push(tree(tag, ..children, child-spacing: child-spacing, layer-spacing: layer-spacing, roof: roof, term: term))
    } else {
